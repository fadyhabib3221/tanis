...
import { Plus, X, Pencil, Trash2, Search, Plane, Download, Store, CheckCircle, Building2 } from "lucide-react";

/* ── Airline codes reused from flights page – 3-digit ticket prefix → Carrier ── */
const AIRLINE_CODES = {
...
export default function SuppliersPage() {
  ...
  const validate = () => {
    ...
    const duplicate = suppliers.find(
      (s) => (s.symbol || "").toUpperCase() === sym && s.id !== editing?.id
    );
    if (duplicate) {
  ...
  const handleSeedAirlines = async () => {
    const total = Object.keys(AIRLINE_CODES).length;
    if (!confirm(`Add ${total} airlines as suppliers with 50.00.00.XXXX code. Duplicates will be skipped. Continue?`)) return;
    setSeeding(true);
    try {
      const snapshot = await getDocs(collection(db, "suppliers"));
      const existing = snapshot.docs.map((d) => d.data());
      const existingCodes = new Set(existing.map((s) => (s.code || "").trim()));
      const existingSymbols = new Set(existing.filter((s) => (s.category || "Airline") === "Airline").map((s) => (s.symbol || "").toUpperCase().trim()).filter(Boolean));
      ...
      for (const [key, info] of Object.entries(AIRLINE_CODES)) {
        ...
        // Skip if code exact match OR symbol already exists for Airline category
        ...
        // update sets so subsequent loop iterations in same run don't duplicate (e.g., duplicate carrier KL for 072/074)
        existingCodes.add(fullCode);
        if (symbol) existingSymbols.add(symbol);
        added++;
      }

      toast.success(`Added ${added}, skipped ${skipped} existing`);
    } catch (error) {
      console.error(error);
  ...
  return (
    <div>
      <Navbar title={t("common.suppliers") || "Suppliers"} />
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            <p className="text-sm text-gray-500">
              {suppliers.length} {t("common.suppliers")?.toLowerCase() || "suppliers"}
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            {isAdmin && (
              <button
                ...
                title="Import airlines as suppliers – 50.00.00.XXXX"
              >
                <Plane size={18} />
                {seeding ? "Importing..." : `Import airlines (${Object.keys(AIRLINE_CODES).length})`}
              </button>
            )}
            <button
              onClick={openAdd}
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
          {loading ? (
            ...
            <div className="p-12 text-center text-gray-500 text-sm">
              {suppliers.length === 0 ? t("common.noData") || "No suppliers yet. Click Add Supplier." : "No matching suppliers"}
            </div>
          ) : (
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b shrink-0">
              <div>
                <h3 className="font-semibold text-lg">
                  {editing ? (t("common.edit") || "Edit Supplier") : (t("suppliers.addSupplier") || "Add Supplier")}
                </h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-auto">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  />
                  <p className="text-[11px] text-gray-400 mt-1">2-5 chars, A-Z0-9, uppercase auto. Shown in Flights Ticket column.</p>
                </div>
